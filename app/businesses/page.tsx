...
import { getBusinessContacts, getBusinessDirectoryRows } from "@/lib/matches";

export const dynamic = "force-dynamic";

export default async function BusinessesPage() {
  const [businesses, contacts] = await Promise.all([
    getBusinessDirectoryRows(),
    getBusinessContacts(),
  ]);
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-8xl flex-col gap-8 px-6 py-10 sm:px-10 lg:px-12 lg:py-14">
      ...
      <BusinessDirectoryTable businesses={businesses} contacts={contacts} />
    </main>
  );
}
