...
function buildDraftHtml(
  ...
  return [
    ...
    `    <li style="margin: 0 0 10px;">The interview should be on a topic that emphasizes ${guestName}\'s expertise and would easily allow for its authority statement(s) to get sprinkled in; however, it must also relate to ${hostName}\'s business since it\'s being published there. To achieve this, you can prompt ChatGPT <span style="font-family: 'Courier New', monospace;">"Write a 400-700 word Q&amp;A style interview post with an [Expert's Name] from ${guestName} in their capacity as [Job Title] at ${guestName}. Relate it to ${hostName}\'s business."</span> Then feed it a brief idea of what topic to cover and it will do the rest.</li>`,
